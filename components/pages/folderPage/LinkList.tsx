...
import { getUserLinks } from '@/apis/api';
import FolderList from '@/components/pages/sharedPage/FolderList';
import UpdateBtnList from '@/components/pages/folderPage/UpdateBtnList';
...
const LinkList = ({ selectedFolderId }: LinkListProps) => {
  const folders = useContext(FoldersContext);
  const [links, setLinks] = useState<Link[]>();

  const selectedFolderName = selectFolderName(folders, selectedFolderId);

  const fetchLinks = async (id: number | string) => {
    try {
